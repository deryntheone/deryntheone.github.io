export class Ten extends Phaser.Scene {
    constructor() {
        super('Ten');
    }

    preload() {
        this.load.image('10photo', 'assets/photos/ten_pic.jpg');
        this.load.image('10background', 'assets/backgrounds/ten_bg.png');
        this.load.image('10task', 'assets/tasks/ten_task.png');
    }

    create() {
        const { width, height } = this.scale;
        
        const background = this.add.image(0, 0, '10background').setOrigin(0, 0);

        const task = this.add.image(1100, 680, '10task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '10photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/16rDeTXRehdw06_Ame4Y_ivzgbzMOvZJA/view?usp=drive_link', '_blank');
        });
