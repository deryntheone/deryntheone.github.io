export class One extends Phaser.Scene {
    ...
    preload() {
        this.load.image('1photo', 'assets/photos/one_pic.jpg');
        this.load.image('1background', 'assets/backgrounds/one_bg.png');
        this.load.image('1task', 'assets/tasks/one_task.png');
    }

    create() {
        const { width, height } = this.scale;
            
        const background = this.add.image(0, 0, '1background').setOrigin(0, 0);

        const task = this.add.image(1100, 435, '1task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '1photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/1cOVUBjr0_pjjLnLiYGQOn0IEUu_mdLPA/view?usp=drive_link', '_blank');
        });
