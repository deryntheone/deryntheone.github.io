export class Eight extends Phaser.Scene {
    constructor() {
        super('Eight');
    }

    preload() {
        this.load.image('8photo', 'assets/photos/eight_pic.jpg');
        this.load.image('8background', 'assets/backgrounds/eight_bg.png');
        this.load.image('8task', 'assets/tasks/eight_task.png');
    }

    create() {
        const { width, height } = this.scale;

        const background = this.add.image(0, 0, '8background').setOrigin(0, 0);

        const task = this.add.image(1100, 600, '8task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '8photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/1RggX3q7tEAcywnFANCRDKzINabt0rfpX/view?usp=drive_link', '_blank');
        });
