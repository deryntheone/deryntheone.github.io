export class Three extends Phaser.Scene {
    constructor() {
        super('Three');
    }

    preload() {
        this.load.image('3photo', 'assets/photos/three_pic.jpg');
        this.load.image('3background', 'assets/backgrounds/three_bg.png');
        this.load.image('3task', 'assets/tasks/three_task.png');
    }

    create() {
        const { width, height } = this.scale;
        
        const background = this.add.image(0, 0, '3background').setOrigin(0, 0);

        const task = this.add.image(1100, 465, '3task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '3photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/1VFfIRW2YiKrI-h44tGN_GOEp-6dYbihc/view?usp=drive_link', '_blank');
        });
