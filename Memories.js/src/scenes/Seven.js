export class Seven extends Phaser.Scene {
    constructor() {
        super('Seven');
    }

    preload() {
        this.load.image('7photo', 'assets/photos/seven_pic.jpg');
        this.load.image('7background', 'assets/backgrounds/seven_bg.png');
        this.load.image('7task', 'assets/tasks/seven_task.png');
    }

    create() {
        const { width, height } = this.scale;

        const background = this.add.image(0, 0, '7background').setOrigin(0, 0);

        const task = this.add.image(520, 225, '7task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '7photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/1OUAuLzZJQ2DDsswQ0v8C4EbsnTdI989v/view?usp=drive_link', '_blank');
        });
