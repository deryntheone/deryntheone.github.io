export class Nine extends Phaser.Scene {
    constructor() {
        super('Nine');
    }

    preload() {
        this.load.image('9photo', 'assets/photos/nine_pic.jpg');
        this.load.image('9background', 'assets/backgrounds/nine_bg.png');
        this.load.image('9task', 'assets/tasks/nine_task.png');
    }

    create() {
        const { width, height } = this.scale;

        const background = this.add.image(0, 0, '9background').setOrigin(0, 0);

        const task = this.add.image(-15, 50, '9task').setOrigin(0, 0);
        task.setInteractive({ useHandCursor: true });
        task.on('pointerdown', () => this.showPopup());

        const photo = this.add.image(width / 2, height / 2, '9photo').setVisible(false);
    ...
    showPopup() {
        ...
        linkBtn.on('pointerdown', () => {
            window.open('https://drive.google.com/file/d/1E9BXaLHaqXkBNm7A7LRSCxz1j1rbpYOH/view?usp=drive_link', '_blank');
        });
